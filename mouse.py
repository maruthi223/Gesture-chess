...
def is_thumb_closed(hand_landmarks):
    """Determine if the thumb is closed (tucked towards the palm)."""
    ...
    # Measure distances
    tip_to_index_mcp = calculate_distance(thumb_tip, index_mcp)
    tip_to_mcp = calculate_distance(thumb_tip, thumb_mcp)

    # Condition: thumb tip is close to palm and below thumb MCP (optional orientation logic)
    is_close_to_palm = tip_to_index_mcp < 0.1
    is_bent_inward = thumb_tip.x < thumb_ip.x if hand_landmarks.landmark[mpHands.HandLandmark.WRIST].x < index_mcp.x else thumb_tip.x > thumb_ip.x

    return is_close_to_palm and is_bent_inward
